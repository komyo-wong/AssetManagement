...
<template>
  <div
    ...
    :class="{ 'has-custom-bg': !!loginBackground }"
    :style="leftStyle"
  >
    <div class="logo">
    ...
    <div v-if="!loginBackground" class="left-img">
    ...
    <!-- 几何装饰元素（自定义背景时弱化） -->
    <div v-if="!loginBackground" class="geometric-decorations">
      <!-- 基础几何形状 -->
...
<script setup lang="ts">
  ...
  const { systemName, loginBackground, branding } = storeToRefs(brandingStore)
  ...
  const leftTitle = computed(
    () => branding.value.loginTitle?.trim() || t('login.leftView.title')
  )
  const leftSubtitle = computed(
    () => branding.value.loginSubtitle?.trim() || t('login.leftView.subTitle')
  )
  const leftStyle = computed(() => {
    if (!loginBackground.value) return undefined
    return {
      backgroundImage: `linear-gradient(180deg, rgb(15 23 42 / 28%), rgb(15 23 42 / 45%)), url(${loginBackground.value})`,
  ...
  .login-left-view {
    ...
    &.has-custom-bg {
      ...
      .text-wrap p {
        ...
        text-shadow: 0 1px 6px rgb(15 23 42 / 35%);
      }
    }
