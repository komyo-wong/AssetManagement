...
<template>
  <div class="layout-content" :class="{ 'overflow-auto': isFullPage }" :style="containerStyle">
    ...
    <RouterView v-if="isRefresh" v-slot="{ Component, route }" :style="contentStyle">
      <!--
        KeepAlive 与 Transition mode=out-in 叠用时，部分嵌套路由切换会出现内容区空白，
        表现为菜单能点但页面“没反应”。这里改为单一渲染分支，避免空白卡住。
      -->
...
<script setup lang="ts">
  ...
  const pageCacheKey = computed(() => `${getUserInfo.value.userId || 'anon'}:${route.path}`)
