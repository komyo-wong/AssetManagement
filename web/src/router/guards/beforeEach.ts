...
import { fetchGetUserInfo, fetchRefreshAccessToken } from '@/api/auth'
...
// 每次页面生命周期只主动探测一次 HttpOnly 刷新会话，避免匿名访问重复请求。
...
async function handleLoginStatus(
  ...
  // Access Token 只驻留内存；页面刷新后通过 HttpOnly Cookie 恢复短期令牌。
...
async function fetchUserInfo(): Promise<void> {
  ...
  useTenantContextStore().hydrate(data.tenantMemberships)
