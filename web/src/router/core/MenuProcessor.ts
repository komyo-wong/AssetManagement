...
export class MenuProcessor {
  ...
  private async processFrontendMenu(): Promise<AppRouteRecord[]> {
    ...
    const permissions = [
      ...(userStore.info?.platformPermissions ?? userStore.info?.buttons ?? []),
      ...tenantContextStore.accessiblePermissions
    ]

    let menuList = [...asyncRoutes]
    menuList = this.filterGeotagMenus(menuList, !!userStore.info?.geotagEnabled)

    // Root 与后端一致：拥有全部能力，菜单不做权限裁剪。
    if (userStore.info?.rootAccount) {
      return this.filterEmptyMenus(menuList)
    }

    // 菜单过滤只是可发现性控制，服务端仍会按当前 tenant/project 上下文重新授权。
  ...
  /**
   * 未接通 GeoTag 时整组菜单不出现（Root 同样隐藏）。
   */
  private filterGeotagMenus(menu: AppRouteRecord[], enabled: boolean): AppRouteRecord[] {
    if (enabled) {
      return menu
    }
    return menu.reduce((acc: AppRouteRecord[], item) => {
      if (item.meta?.requiresGeotag) {
        return acc
      }
      const filteredItem = { ...item }
      if (filteredItem.children?.length) {
        filteredItem.children = this.filterGeotagMenus(filteredItem.children, enabled)
      }
  ...
  private filterMenuByAccess(
    ...
    return menu.reduce((acc: AppRouteRecord[], item) => {
      ...
      const hasRole = !itemRoles || itemRoles.some((role) => roles.includes(role))
      const hasPermission =
        !itemPermissions ||
        (item.meta.permissionMatch === 'all'
          ? itemPermissions.every((permission) => permissions.includes(permission))
