...
export const useAuth = () => {
  ...
  const hasAuth = (auth: string): boolean => {
    ...
    if (isFrontendMode.value) {
      const permissions = new Set([
        ...(info.value?.platformPermissions ?? info.value?.buttons ?? []),
        ...tenantContextStore.currentPermissions
      ])
      return permissions.has(auth)
    }
  ...
  const hasAnyAuth = (permissions: readonly string[]): boolean => permissions.some(hasAuth)

  const hasAllAuth = (permissions: readonly string[]): boolean => permissions.every(hasAuth)
